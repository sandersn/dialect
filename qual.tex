...
\namecite{goebl06} provides a summary of his work, including
...
% Take as an example the distance between the source ``cab'' and
% the target ``cob''. The three cases are as follows:

% \begin{enumerate}
% \item Insertion: lev(``cab'',``cob'') = ins(`b')+lev(``ca'',``cob'').
%   This is a particularly bad match, because the recursive call to lev
%   costs 3: ``ca'' and ``cob'' only share one character. The total cost
%   is 4=1+3 since insertion costs 1.
% \item Deletion: lev(``cabb'',``cob'') = del(`b')+lev(``cab'',
%   ``co''). This option is even worse; the cost is still 4 because deletion costs
%   1 and the distance between ``cab'' and ``co'' is 3. But the answer
%   is also wrong because the output is ``ca'' because `b' is deleted.
