...
in section \ref{feature-ranking}.

\subsection{Correlation with Corpus Size}

Correlation with corpus size is a confounding factor.

This can be measured in two ways: by total size of the two regions
being compared, or by difference in size. Right now I'm using total
size because larger totals indicate more tokens, thereby driving up
token count differences compared to comparison between small regions.

Difference in size is appropriate if you decide that the normalisation
might not be doing its job with respect to sentence length. But this
shouldn't be detectable by using difference in size. So that's just
wrong.

TODO:Move this to methods if I decide to keep it.
...
\end{tabular}
 \label{size-cor-1000-freq}
  \caption{Size correlation for sample size 1000, Frequency
    normalisation only}
\end{table}
...
Considering how high the correlation with size is with the ratio
normalisation included, the rest of this chapter will include results for
ratio normalisation only in special cases. Most examples will use only
frequency normalisation.

Note that cosine still comes up looking just fine. Strange. Still need
to investigate this.

Of course, corpus size correlates with travel distance at 0.32 and
geographic distance at 0.31, for $p
< 0.01$. So I don't know how to interpret this really. It could be
that corpus size SHOULD correlate with distance because of a
difference between city and country. But this has 3 problems; I notice
this rift in the parameters that DON'T correlate with size; the city
corpora are not necessarily the larger ones (though I could measure
that); and it still doesn't explain why travel distance correlates with
corpus size.
...
dissertation after I get a copy to read).

% \subsection{Cluster Analysis}

% TODO: This is old. Also it goes in the DISCUSSION chapter.
...
other. Cluster B contains Jamshog, Ossjo and Tors\.as. In the figures,
