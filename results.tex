...
not be significant. Therefore the results should have few to no
...
\end{tabular}
 \caption{Number of significant distances for complete sites, 1
   normalization}
 \label{sig-1-full}
\end{table}
...
\end{tabular}
 \caption{Number of significant distances for complete sites, 5
   normalizations}
 \label{sig-5-full}
\end{table}
...
has similar performance. For full-site comparisons, both perform
considerably worse; surprisingly, both perform better on unigram
features, Jensen-Shannon so much so that it is the only feature set for
...
Correlation between measures indicates that they produce similar
...
input features differently than the other measures.
...
The inter-measure correlation is essentially a reflection of the
...
obvious sub-trees which can be treated clusters. By grouping sites
...
is easy to argue for, but outliers like Anundsj\"o and \.Arsunda are
...
detail but the trees with more clusters do not contradict the clusters
of the flatter trees.
...
them. The orientation of the sites may differ between runs of the MDS
algorithm, but Kruskal's method guarantees that its results will be
comparable given some rotation. Equivalent MDS maps may be rotated
with respect to each other in 3D space, and this rotation is visible
in the color selection: if two sites are both blue in one map and in
another map are both orange, then they have the same relation to each
other.
...
\ref{mds-1-1000-js-trigram} -- \ref{mds-5-1000-js-trigram}) all show
...
\begin{tabular}{c|c||c|c}
 ...
 $++$ & coordinating conjunction&MV & verb ``m\.aste'' (must) \\
 AB & adverb&NN & noun \\
...
EN & indefinite article&RO & numeral \\
 FV & verb ``f\.a'' (get) &SP & present participle \\
...
\ref{clusterA-clusterD-feat-5-1000-trigram-ratio}). The same is of the
other clusters for the most part. Unfortunately, this makes it hard to
say interesting things about the difference in feature
distribution. It does appear that clusters B and C have heavier use of
adverbs and of conjunctions; for example $++$-AB-AV in in figure
\ref{clusterA-clusterC-feat-5-1000-trigram-ratio}. The comparison
between cluster A and cluster B even highlights the trigram AB-AB-AB
(figure \ref{clusterA-clusterB-feat-5-1000-trigram-ratio}) as
important.

% TODO: Discuss the ending in IP. Ends of sentences? Sentences are
% shorter? (I hope the second is false.)
