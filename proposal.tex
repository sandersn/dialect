...
And now for a diversion on phonology. Dialectometry has focussed on
phonology, while syntax has remained relatively undeveloped. The
reason for the focus on phonology is that it (1) is simpler to measure
with high precision and (2) contains more dialect difference
information. The lexicon also contains many dialect differences, but
does not have a simple high-precision distance measure.
String alignment, Levenshtein distance, is a
well-understood algorithm used in many fields. It is symbolic, and its
intermediate data structures are fairly easy to interpret. I believe
...
Actually I should re-order the previous paragraph to refer to `more
differences' first, then Levenshtein distance. I think the theory
proposed is interesting but needs lot of backup. This will require
going back to those papers I read in Ken's class and then building
from there.
...
The previous work described above leads to research questions, which in
turn lead to the four hypotheses given below.
