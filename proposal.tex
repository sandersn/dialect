...
Its primary goal is to characterise the linguistic features that
...
and combine linguistic features. In recent years especially it has
...
the features in a mathematically and linguistically sound way. With
respect to distance measures, dialectometry has focussed on phonology,
while syntax has remained relatively undeveloped. An important force
behind the focus on phonology is that simple methods are easily
defined both for linguistic decomposition and mathematical
combination. Words decompose to segments and segments, if necessary,
further decompose to phonological features. This decomposition is not
terribly controversial and, significantly, is the primary obvious one
(modulo features tiers).  For combination, string alignment,
Levenshtein distance, is a well-understood algorithm used in many
fields. It is simple mathematically, and has the additional advantage
that its intermediate data structures are fairly easy to interpret
linguistically (aka symbolically).
...
Previously, the state of the art for syntax has also been Goebl's WIV
but at a level even worse than the lexicon: specific pre-defined
features. This limits a computational method by requiring the
dialectologist do some analysis first to find syntactic dialect
markers. This is a bootstrapping step that would better be
avoided. However, it points toward a better way for syntax: unlike words,
sentences are complex and decomposition is easy. Unlike phonology, the
right way is not {\it obvious}, but decomposition methods are easy to
come up with. If a useful sentence decomposition can be specified,
then perhaps a more useful aggregate measurement method can also be
specified, one that takes into account properties of syntax similar to
the way that Levenshtein distance does when specified with edit
functions that are weighted according to phonological feature
differences.
