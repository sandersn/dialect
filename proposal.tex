...
feature, I will use deleted interpolation (or something equally
simple).
TODO

For backoff between types of features, I will use ranked combinations
of feature sets, based to Martin Volk's system (TODO:CITE) for verb
attachment. Volk used XX reliability for ranking quality of combined
feature types; I will use number of significant region differences for
ranking: the most reliable feature type will be the one that produces
the highest number of significant distances between
regions. Combinations of feature types will be ranked by averaging the
number of significant distances that the constituent feature types
obtain.

Volk also trained something using NEGRA for supervised training.
Do I need to talk about training the backoff here? I'm not sure what
the confidence threshold for each feature-type set is.

To summarise, if one combination of feature types can't produce a
significant difference (OR does not reach the confidence threshold),
