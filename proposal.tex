...
possibility is supertags (TODO:CITE) which are similar to
leaf-ancestor paths but include more context from the upper structure
of a syntax tree.
% or supertags. (Should CITE at least supertags because I think they are
...
syntax distances. These features can be discovered by
comparing performance of a number of different feature sets on a fixed
corpus. In addition, combinations of successful features will
produce even better performance.
% What? That doesn't make sense; they aren't the most successful
% features if they can be combined to get MORE successful features.
The quality of a set of features can
be measured by its sensitivity---the number of significant distances
it finds---and the relevance of the highly ranked features $R$
produces.
...
speakers. Currently there are 36713 sentences of transcribed speech
from 49 sites, an average of 749 sentences per site. In order to
detect significant differences, the sites may need to be grouped by
province; previous work on British English used Government Office
Regions with at least 850 sentences per region.
...
Because SweDiaSyn consists of untagged lexical items, Talbanken05 will be
used to train a POS tagger and parsers. Talbanken05 is an updated
version of Talbanken76, a Swedish treebank of roughly 300,000 words
\cite{nivre06}. The treebank consists of written and spoken
Swedish. The original treebank was annotated according to a custom
scheme called MAMBA; Talbanken05 updates this to include phrase
...
parsed. In this case, both phrase-structure parsing and dependency
parsing will be needed.
...
% TODO: CITE The paper that shows this.
