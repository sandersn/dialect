...
features. This decomposition is straightforward and based on Chomsky
and Halle (1968) (TODO:Cite this, I can't believe it's not in my
bibtex file). For combination, string alignment, or Levenshtein
...
fairly easy to interpret linguistically.

\begin{equation}
d(F_S,F_T) = \sum_{s,t \in S, T} lev(F_S,F_T)
\label{d-example-phonology}
\end{equation}

\begin{figure}
\[\xymatrix{
  & S = \textrm{corpus} \ar@{>}[dl] \ar@{>}[d] \ar@{>}[dr] &\\
  \ldots & \textrm{[k\~an]} \ar@{>}[d] & \ldots \ar@{>}[d] \\
  &s = \textrm{[k] [\~a] [n]} \ar@{>}[d]\ar@/_7pc/[ddd]  &
  t = \ldots\ar@{>}[d] \ar@/^7pc/[dddl] \\
  &s_0 = \left[ + cons \ldots \right] \ar@{>}[d] & t_0 = \ldots\ar@{>}[dl]  \\
  & d(s_0, t_0)  \ar@{>}[d] & \\
  & lev(s, t) \ar@{>}[d] & \\
  & \sum_{S \times S} & \\
} \]
\label{phonology-distance-measure-model}
\caption{Phonology Distance Measure Model}
\end{figure}
...
\label{syntax-distance-measure-model}
\caption{Syntax Distance Measure Model}
\end{figure}

A second reason behind dialectometry's neglect of syntax is that
dialectology at large tends to focus on phonology.
This might be solely to due to the history of
dialectology as a field, but it is more likely that more phonological differences
exist between dialects than syntactic differences. Whatever the
reason, there is much less work to compare with results of syntactic
dialectometry, and much less inspiration to be had from the
literature. Note that lexical dialectometry suffers from the first
problem: no good, accepted method of decomposition; but not the
second: lexical analysis is simple in dialectology and there has been
a fair amount of it.

I believe that a second driving force is that
there is also more dialect difference at the phonological level
(actually, that there is less difference at the syntactic level), so
the best results will be initially found there.
In addition, [I believe that] phonology contains more dialect
difference information than syntax. While the lexicon also contains
many dialect differences, lexical items do not have an obvious
decomposition. Actually I should re-order the previous paragraph to
refer to `more differences' first, then Levenshtein distance. I think
the theory proposed is interesting but needs lot of backup. This will
require going back to those papers I read in Ken's class and then
building from there.

One reason I believe phonology to contain more differences is
that the literature discusses many more pronunciation
characteristics. Although it is possible that only phonologists become
dialectologists, it is more likely that dialectologists discuss what
they find. Second, and completely unjustified, is my theory of dialect
differences (and in fact language change as a whole) as a series of
random changes caused by the production apparatus. Some changes are
perceptable by some people. Some of these perceived changes are taken
to be socially important and are imitated, so that a change spreads
and eventually is regularised. In this theory, phonological changes
spread more easily because of the underlying theory that phonology is
more susceptible to variation in production. Therefore, dialects
should have more phonological variation. Note that this theory does
not account for all sources of variation; lexical differences also
arise from differing environments with different physical components.

Although there are probably as many lexical differences between
dialects as phonological differences, there is no high-precision
measure here. Instead, Goebl's Weighted Identity Value (WIV) represents the
state of the art. WIV is more complex mathematically than Levenshtein
distance, and takes whole words as input. The problem is that there is
no obvious way to decompose words to gain additional information. In
contrast, phonological words have an obvious decomposition into
segments and segments into phonological features.

Before Nerbonne \& Wiersma's measure, the state of the art for syntax
distance was Goebl's WIV, but only over specific pre-defined
features. WIV is a sophisticated means of composition, helpful for
small corpora, but manually extracted features are deficient compared
to automatic extraction of features. It is limited by requiring preliminary
dialectologic analysis in order to find syntactic dialect
markers. Manual dialectologic analysis nullifies the entire point
of dialectometry---it is impoverished and encodes human bias directly.

However, syntactic structures are not difficult to decompose. Unlike
phonology, the right way is not {\it obvious}, but decomposition
methods are easy to come up with. If a useful sentence decomposition
can be specified, then perhaps a more useful aggregate measurement
method can also be specified, one that takes into account properties
of syntax similar to the way that Levenshtein distance does when
specified with edit functions that are weighted according to
phonological feature differences.

A third problem, related to the second, is that there is comparatively
little dialectology published about syntax.
% \subsection{Problems}

Syntax is a relatively undeveloped area in dialectometry. This
dissertation will continue the development of a syntactic dialectometry
literature. The biggest deficiency in the literature is the absence of a generally accepted
syntax measure. In contrast, Levenshtein distance is widely accepted
as the standard phonological distance against which new distance
measures are compared. It is supported by a large body of literature
that shows it to be consistent for many languages and to detect the
same linguistic features analysed by dialectologists
\cite{heeringa04}. There is no equivalent to Levenshtein distance for syntax.

Analysis of syntactic data poses different challenges
...
the Syntactic Atlas of the Dutch Dialects \cite{barbiers05} using the content-agnostic
...
corpus, which in turn implies a statistical method. % probabilistic ?
...
% NOTE: This is a copy of stuff I've said elsewhere and should be
% integrated there instead of having its own paragraph. The problem is
% that Sandra didn't find it obvious enough, not that it wasn't there
% at all. So I need to use her terminology and then expand my existing
% text.
Additional problems: phonology is 1-dimensional, with one obvious way
to decompose words into segments and segments into feature. Syntax is
2-dimensional. The best decomposition is not so obvious, although
decompositions are easy to specify. Additionally, the decomposition
must take several more factors into account in order to make sure that
the features it produces are useful and comparable to each other. And
those features are \ldots

Additionally, Levenshtein distance can operate well on traditionally
small, focussed dialect corpora. This simple means of combining
differences, combined with the simplicity of decomposing phonological
data, contrasts strongly with syntax data. In fact, small, focussed
dialect corpora are not well-suited to syntax work, not only because
traditional dialectology methods are not well-suited to the unknown
features of syntactic dialect variation (they must collect small
amounts of data, so the much larger search space of syntactic
variation does not allow significant differences to emerge), but also
because there are existing syntactically annotated corpora that are
not carefully collected dialect interviews.

%%% Leftover previous introduction %%%
...
several research questions remaining. The most important is whether
$R$ is a good measure of syntax distance. Specifically, have the
...
for syntax. Its features will need to be enumerated and ranked, then
compared to the syntactic dialect features found by dialectologists.

Furthermore, if $R$ is found to be a bad measure of syntax distance,
this research will propose and evaluate alternative syntactic distance
measures. Specifically, $R$ is one way to aggregate features that are
created by decomposing sentences. It treats features as atomic, not
using any internal information. As such, it is not much different than
Goebl's WIV. This may not be a problem if the decomposition methods
used to generate features adequately capture dialect differences in
independent, atomic features. If dialect differences cannot be
captured by independent, atomic features, then a more complex method
of combination may be needed to replace $R$.
...
A third question is whether $R$ agrees with
phonological distances measures like Levenshtein distance.
Unlike agreement with traditional dialectology,
there is no {\it a priori} reason to expect agreement between phonology and
syntax in delineating dialect boundaries. However, agreement with
phonological distance would be further evidence for the utility of $R$
as a syntactic distance measure.

\subsection{Hypotheses}

Hypothesis 1: $R$ will agree substantially with the results of
syntactic dialectology. This will be tested by comparing $R$'s results
to the syntactic dialectology literature. Where dialectologists found
differences between regions, $R$ should find a significant difference
as well. In addition, the features found by dialectologists should
agree with the highly ranked features used by $R$ for classification.
Hypothesis 1 is a syntactic parallel of Levenshtein distance with phonology.

Hypothesis 2: Given a syntactically annotated corpus collected for dialectology, $R$
...
better results than corpora without these properties. This will be
tested by comparing results to previous work on corpora that did not
have good dialect coverage. The results of this study should correlate
better with geographical distance than the results of previous
studies. Hypothesis 2 is
an analogy to Gooskens' work on Norwegian as well as intuitions from
computational linguistics.

Hypothesis 3: Given certain features, $R$ will produce more accurate syntax distances
for a given set of subcorpora. The best features can be discovered by
comparing performance of a number of different feature sets on a fixed corpus.

Hypothesis 4: A phonology corpus and syntax corpus based on the same data will
