...
% OK, reorganise this section as follows:
% 1. Merge questions and hypotheses. Put a punchy statement of each
% hypothesis up front, I guess, then follow up with some explanation
% of the question. Or maybe the opposite of that.
% Re-arrange the methods section to mirror the hypotheses. ie,
% Hypothesis 1: First I will do this, this, this and this. Next:
% Hypothesis 2: first two same as H1, then I will do this.

The state of syntax measures in dialectometry described above leaves
several research questions remaining. The most important for this
...
for syntax. The syntactic features it ranks highest should be
...
using any information specific to syntax. As such, $R$ is not much
...
the highly ranked features used by $R$ for classification.  Hypothesis
1 will be tested by comparing $R$'s results to the syntactic
dialectology literature. First, and most obviously, the broad regions
accepted by dialectology will be reproduced by the classifier. For
example, my previous research on British English reproduced the
well-known North England-South England distinction. In addition, $R$
...
regions, $R$ should find a significant difference as well. What's
more, the specifics of the difference from dialectology should be
reflected in the high-ranked features used by $R$ to distinguish that
region from others.

Hypothesis 2: The best features for $R$ will produce the most accurate
syntax distances. These features can be discovered by comparing
performance of a number of different feature sets on a fixed
corpus. In addition, combinations of successful features will produce
even better performance.  The quality of a set of features can be
measured by its sensitivity---the number of significant distances it
finds---and the relevance of the highly ranked features $R$ produces.
% TODO: Should really define relevance, eg relevance wrt dialectology
...
boundaries correlate. Hypothesis 3 will be tested by comparing results to
my previous work on separate corpora. There, no significant
correlation was found between phonological distance and syntactic
distance. If significant correlation is found by using the same
corpus for both measures, it indicates that boundaries do coincide but that the
agreement is weak enough to be lost when using corpora collected from
different populations of the same dialects.
% (or different times/social groups)
