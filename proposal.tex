...
If $R$ is found to be a bad measure of syntax distance,
...
captured by independent, atomic features, then a more syntax-specific
method of combination will be needed instead. Alternatively, a more
complex statistical measure may be useful, taking the basic of
direction of $R$ and increasing its sensitivity. Kullbeck-Leibler
divergence, though not syntax specific, TODO: XXX.
...
as coarser node tags or bigrams. Features from dependency parses may
...
produce even better performance. The quality of a set of features can
...
corpus, that indicates that boundaries do coincide but that the
agreement is weak enough to be lost when using corpora collected from
different populations of the same dialects. (or different times/social groups)
