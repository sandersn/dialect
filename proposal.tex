...
To investigate this, I propose Hypothesis 1: The features found by dialectologists will agree with
the highly ranked features used by $R$ for classification. I will test Hypothesis
1 by comparing $R$'s results to the syntactic
dialectology literature on Swedish. First, and most obviously, the broad regions
of Sweden accepted by dialectology will be reproduced by the classifier. For
example, my previous research on British English reproduced the
well-known North England-South England distinction.
This research will eliminate the corpus
variability in that research \cite{sanders08b} that resulted in the
...
for syntax.
The syntactic features it ranks highest must be
enumerated, then compared to the syntactic dialect features found by
dialectologists.

According to hypothesis H1A, $R$
will agree substantially with the between-region differences of
syntactic dialectology--where dialectologists find differences between
regions, $R$ should find a significant difference as well.
What's
more, the features found by dialectologists should be
reflected in the high-ranked features used by $R$.
...
syntax distance measure is established, is what features provide the best
...
Therefore, the second hypothesis to be investigated is that the best
features for $R$ will produce the most accurate syntax
...
Therefore, the third hypothesis to be investigated is whether a
phonology corpus and syntax corpus constructed from the same data will
provide better correlation between phonology and syntax distance
measures than a phonology corpus and syntax corpus drawn from
different data sources. I will test this hypothesis by comparing
results to my previous work on British English phonological and
syntactic corpora; there, no significant correlation was found between
the regions extracted from the two corpora. If significant correlation
is found by using the same set of data for both measures, it indicates
that boundaries do coincide but that the agreement is weak enough to
...
If $R$ is found to be a bad measure of syntax distance, this research
...
\section{Methods}
% Label each with section number
To investigate the first (main) hypothesis, I need a dialect corpus
that can be syntactically annotated (\ref{syntactically-annotated-corpora}),
preferably automatically. This
will require a syntactically annotated training corpus
(\ref{syntactically-annotated-training}) and a
parser (\ref{parsers}). Syntactic features must be extracted
(\ref{syntactic-features}), distances between
regions produced (\ref{nerbonne06}) and tested for significance
(\ref{permutationtest}), and the results
clustered (\ref{cluster-analysis}) to determine which dialect regions are found by the
corpus. Finally, the most highly ranked features used to produce the
dialect distances must be enumerated (?.?).

To investigate hypothesis 2, I need a framework for combining features
(?.?) and backing off sparse features (?.?). I also need a way to
generate new features based on other theories of syntax (?.?).
...
measure. I don't have one of these, but there are quite some
possibility, like Kullbeck-Leibler divergence or maximum (?) entropy
...
To investigate hypothesis 3, I need a phonological corpus and a method
...
(Sanders 2008) for details.

\subsection{Corpora} % This is not a good subsection for the new organisation
\label{syntactically-annotated-corpora}
To investigate hypothesis 1, I need a dialect corpus, one that will
contain more differences than the ICE-GB. For this I will use \ldots
:
...
In order to investigate hypothesis 1 by sorting the speakers into
various groups, then letting the computer cluster those groups into
larger clusters. The larger clusters should match linguist
expectations.
:
...
:
In order to investigate hypothesis 1, I will need a training corpus of
Swedish to train these various parsers. Part of speech tagging will
also be needed to run the dependency and constituency parsers.
:
Because SweDiaSyn consists of untagged lexical items, Talbanken05 will
be used to train a POS tagger and parsers. Talbanken05 is a treebank
of written and transcribed spoken Swedish, roughly 3000,000 words in
size. It is an updated version of Talbanken76 \cite{nivre06};
Talbanken76's trees are annotated following a custom scheme called
MAMBA; Talbanken05 adds phrase structure annotation
and dependency annotation using the standard annotation formats
TIGER-XML and Malt-XML.
...
internal nodes.
% TODO: Yeah, the rest of this is pretty cursory. I should add more
% detail all over. At least, I think I should. I can't remember what
% Sandra told me, for some reason I think she mentioned that I should
% not give so much detail? I think I am misremembering.

\subsection{Parsing}
\label{parsers}
In order to identify the features needed for building the language
models described above, SweDiaSyn will need to be POS tagged and
parsed. For this dissertation, both phrase-structure parsing and dependency
parsing will be needed to provide their respective features to the
classifier.
...
In order to investigate hypothesis 1, I'll need a way to tell whether
two corpora are significantly different, and what makes them
different. A permutation test does this.
...
In order to investigate hypothesis 1, I will need to find the clusters
that the distance measure forms of the regions. I will use binary
hierarchical bottom-up clustering to do this.
...
\subsection{Everything for Hypotheses 2 and 3}
In order to investigate hypothesis 2, I will need a framework for
combining feature and backing off sparse features. There is some math
for this that I need to look up in Manning and Schutze or maybe
Jurafsky and Martin.

I also need a way to generate new features. Maybe other syntactic
theories like supertagging would be useful. I already said this
somewhere.
...
How to compare phonological distance with syntactic distance (3). How
to um, DO phonological distance. That's like a whole qualifying paper
on its own. Well, not really, but half of one at least. Maybe a
dissertation chapter in direst need ok.
