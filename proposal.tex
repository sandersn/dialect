...
dissertation will address holes in the syntactic dialectometry
literature. The biggest lack is the absence of a generally accepted
...
that it is noticing % TODO: 'is noticing' is not quite accurate. Anthropomorphism!
dialect differences, but as yet there has not been
a satisfying correlation of its results with phonology or, as with
phonological distance, with existing syntactic results from the dialectelogy
literature.

The initial paper using $R$ for syntax distance was
\namecite{nerbonne06}. Their experiment compared two generations of
Norwegian L2 speakers of English. Nerbonne and Wiersma used $R$ from
\namecite{kessler01}, together with the test for statistical
significance. As input they used part-of-speech trigrams. They found
that the two generations were significantly different, although they
had to normalise the trigram counts for differences in sentence length
and complexity. However, showing that two generations are
significantly different with respect to $R$ does not necessarily
imply that the same will be true for two other language
varieties. Differing generations of L2 speakers is not the same as two
different dialects.

\namecite{sanders08b} addresses this problem by measuring $R$ between
the Government Office Regions of England, using the International
...
distance. It is not clear whether the significant $R$ distances
correlate with dialectometric phonological distance or with known
features found by dialectologists.
% TODO: Might explain some more problems with my research.
...
several research questions remaining. The most important is whether $R$ is a
good measure of syntax distance. Specifically, have the
...
results. In addition to determining whether $R$ is internally
consistent, it is not yet clear whether it agrees with traditional
dialectology results for syntax. $R$'s features will need to be
extracted and ranked, then compared to the syntactic dialect features
...
A third question is whether syntactic distance agrees with
phonological distance. Unlike agreement with traditional dialectology,
there is no a priori reason to expect agreement between phonology and
syntax in delineating dialect boundaries.
...
Hypothesis 1: $R$ will agree substantially with the results of dialectology with respect
to syntax. This will be tested by extracting the highly ranked features
that $R$ uses for classification. The features should be the same as
those analysed by dialectologists. Hypothesis 1 is a syntactic
parallel of Levenshtein distance with phonology.
...
better results than corpora without these properties. Hypothesis 2 is
