...
combining features of different types. The most obvious approach
should work, dumping them all into the same bucket, because they are
just weights and features of the same type {\it already} contain
redundant information. If this turns out to be insufficient, I will
find a more complex and sound way to combine features.
...
% -- but you have to estimate them empirically using held out data
%
% check Jurafsky & Martin under 'smoothing'
...
backing off sparse features. For inter-type backoff I will use deleted
interpolation (or something equally simple).

For intra-type backoff I will use an (ad-hoc?) ranked rule set similar
to Martin Volk's system for verb attachment. He used XX reliability
for ranking quality of rules; I will use number of significant region
differences for ranking: the most reliable feature type will be the
one that produces the highest number of significant inter-region
distances when tested using just that one type of feature. Volk also
trained something using NEGRA for supervised training.

To summarise, if one combination of feature types can't produce a
significant difference, the next one will be tried. The ranking will
be the linear combination (the sum) (or maybe the average?) of the
individual feature types used for that combination.

\subsection{Alternate Feature Sets}
\label{alternate-feature-sets}
% Extended feature sets instead of completely basing them on another
% syntactic theory. In other words, maybe generate additional context
% for leaf-ancestor paths, but it need not be based on CCG, just on
% ad-hoc context widening. That I make up eh.
...
head. Something similar based in constituents is possible for
leaf-ancestors; dependency paths already include such ``upper
context'' in the form of lexicalised nodes, but each node could be
enriched with surrounding lexical information, creating bigrams or
trigrams of each node.

\subsection{Alternate Distance Measures}
\label{kl-divergence}
In the case that $R$ does not reach statistical significance, I will
need to experiment with similar
but more complicated distance measures, in the chance
that one will be more sensitive. A good first option is
Kullbeck-Leibler divergence, or relative entropy, which is described
in . Besides this, several close variants of relative entropy exist, such as
Jenson-Shannon divergence, that lift various restrictions from the
input distributions.
...
More exotic classifiers are of course possible, although currently I
have not investigated them. Examples are k-nearest
