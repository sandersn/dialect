...
standard dialect than syntactic ones (TODO:Cite, probably
...
approaches do not apply. For one, there are fewer differences to be
found in syntax, and they occur more sparsely. (TODO: Back this up either
with reasoning or citation). However, dialectology has traditionally
worked with fairly small corpora. This suffices for phonology, because
it is easy to extract good features and there are many
consistent differences. For syntax, though, it is not possible (TODO: Weasel a
bit) to identify reliable features in small corpora.
...
available, where extraction of individual syntactic features from
carefully elicited sentences is not possible. However, combining these
two facts cancels the problems of both: large corpora should provide
enough evidence to properly rank automatically extracted features so
that the best syntactic decomposition can be identified.
