...
is inherited from dialectology at large (TODO:Cite?). This might be
solely to due to the history of dialectology as a field, but it is
likely that more phonological than syntactic differences exist between
dialects. This is probably due to historically greater standardisation
of syntax via the written form of language; phonological
dialect features are less likely to be stigamatised and suppressed by a
standard diaect than syntactic ones (TODO:Cite, probably
Trudgill and Chambers something like '98, maybe where they talk about
what aspects of dialects are noticed and stigmatised). Whatever the reason, much
less dialectology work on syntax is available for comparison with new
dialectometry results.

\subsection{Problems}

Syntax is a relatively undeveloped area in dialectometry.
Currently, the literature lacks a generally accepted syntax measure.
The previous section introduced the reasons for this deficiency.
Unfortunately, trying to approach the problem by copying phonology
does not work very well. The real problem is that there are fewer
differences to be found in syntax, and they occur less often (TODO:
Back this up either with reasoning or citation). But traditionally,
dialectology has worked with fairly small corpora. This suffices for
phonology, for which it is easy to extract good features and there are
many consistent differences. However, it is not possible (TODO: Weasel
a bit) to extract good enough features on such a small corpus.

There are two approaches that have been proposed to remedy this. The
first, proposed by \namecite{spruit08} for analysing the Syntactic
Atlas of the Dutch Dialects \cite{barbiers05}, is to use manual
feature extraction so that only the most salient features are
used. Then a sophisticated method of combination such as Goebl's
Weighted Identity Value (WIV), described below and by
\namecite{goebl06}, can be used to produce a distance. WIV is more
complex mathematically than Levenshtein distance, and takes any kind
of item as input.  The main problem with this is that manual feature
...
Besides this problem, it ignores two important aspects of the problem:
first, syntactic structure IS easily decomposable. Second, many large
syntactic corpora exist that would be impossible to analyse using only
manual features. Given a good classifier, it should be possible to
propose enough features and then automatically winnow them down to the
best-performing ones.

In short, although a good solution to the syntax distance problem is
not {\it obvious} from syntactic structure and large syntactically
annotated corpora, a good solution can be extracted {\it
  automatically} by trying many different decomposition methods and evaluating
them automatically.

In addition, the large corpora will enable
discovery of many types of differences, not just ones that have been
discovered previously by dialectologists.

Even more, perhaps it will be possible to define a better method of
combination which takes syntactic information into account. Um, this
is really baseless speculation, but WIV isn't that good and R isn't
either. A learning classifier MIGHT do better but WHO KNOWS. Not me,
yet, that's for sure.

% TODO: Old but some useful stuff could be folded in probably
However, for most languages, large, unstructured syntactic corpora are the norm;
