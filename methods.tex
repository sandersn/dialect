...
number. \namecite{kessler01} creates a simple measure called the
...
$B$.
\[ R = \sum_i |c_{ai} - \bar{c_i}| \textrm{ where } \bar{c_i} = \frac{c_{ai} + c_{bi}}{2}\]
...
comparable lengths between the two types of paths.

In addition, leaf-head paths capture long-distance
context nicely, like enhanced trigrams. Not as much upper structure,
unless you have a deeply embedded sentence, in which case you can
see that the outer verb, etc, mimic the upper structure of a
leaf-ancestor path. Ultimately, leaf-head paths are just pretty cool.
...
are quite a few possibilities; Kullbeck-Leibler divergence is one
example.
...
with features. Then the features can be
...
http://www.mathpsyc.uni-bonn.de/doc/delbeke/delbeke.htm says that
metric MDS is a special case of PCA anyway. Not sure whether I'm doing
metric MDS (though I guess so) because I don't know if my analysis is
unique on the interval level (or ratio/absolute level).
...
Different feature sets capture different aspects of the underlying
syntactic structure. Ideally, one feature set would capture so much
that no others would be needed. However, in the absence of a
minimalist parser capable of returning the complete syntactic
structure of Swedish (Josh's dissertation, not yet finished), multiple
feature sets are useful in an empirical setting. One set may capture
information that the rest miss.

This section describes the variants besides the main feature sets
already described above: leaf-ancestor paths and dependency
paths. Most are variants on these two main sets.
...
\ref{psg-example-tree} the extracted relations are given in
...
This feature set emphasises upper tree structure more than other
feature sets because features from the upper tree are not only
represented (as they are not in dependency-based and trigram-based
features), but also given their own features separate from the
features starting lower in the tree.
...
This variant adds some of the vertical information present in
leaf-ancestor paths, hopefully without introducing data sparseness
problems (not measured). However, it retains its advantage over
...
Another variation for generating dependency features for MaltParser is
to use Timbl, the Tilburg Memory-Based Learner, as the oracle classifier
for Nivre's dependency parsing algorithm than the default support
vector machines.

This is, however, somewhat complicated since Timbl is quite sensitive
to parameters and will exhibit better performance if the parameters
are tuned for the particular task of Swedish parsing. To find the best
...
Talbanken. The best combination was used as a basis for parsing and
parsing proceeded identically to previous runs.
...
% TODO: Replace Kullbeck with Kullback

Kullbeck-Leibler divergence, or relative entropy, is described in
...
However, Kullbeck-Leibler divergence as defined is a divergence: it
...
Here is where I describe what kinds of limitations Jensen-Shannon
lifts from Kullback-Leibler, besides the obvious initial design as a
dissimilarity rather than a divergence.


\subsubsection{Cosine similarity}

Cosine similarity is a standard in many parts of computational
linguistics. It captures the dissimarlity between two high-dimensional
points in space. Type counts of features can be modelled as points on
a dimension, with each feature a dimension. The result is:

\begin{equation}
  \frac{\sum_i {c_{ai}c_{bi}}} {\sqrt{\sum_i {c_{ai}}} + \sqrt{\sum_i{c_{bi}}}}
  \label{cosmeasure}
\end{equation}

This discussion is not done because I don't compare my use to existing
uses, nor do I justify my use. I should be able to do a little
research to find good reasons but at this point, my use is justified
mainly because so many other people use it. Specifically, Wybo.

Interestingly, the results are very different from other methods,
possibly because it deviates from the simple linear sum that the other
methods use.

% \subsubsection{LSA} Nope, didn't work out

\subsubsection{Alternate Training for MaltParser}
