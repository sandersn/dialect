...
To account for differences in corpus size, sampling with replacement
is used. The corpus is sampled by sentence rather than by feature
because the intent is to capture syntax, where the composite unit is
the sentence. In other words, phonology's composite unit is the
word---most processes operate within the word on individual features;
some processes operate between words but they are fewer. In phonology,
the assumption that words are independent will capture many of the
interesting phonological processes. In the same way, the basic unit of
analysis is the sentence; processes operate on the words in the
sentence, but inter-sentence processes are fewer. Because of this, the
corpora are sampled by sentence, combining the sentences of all
speakers from an interview site into a single corpus. This removes the
per-speaker sampling of \quotecite{wiersma09} work on Finnish L2
speakers. Although the motivation is partly theoretical, there is also
a difference between the Swediasyn, with 2-4 speakers for each site of
30, and Wiersma's corpus, with dozens of speakers but only two
groups. I assume that, since discovery of dialect features are the
goal of this research, combining the sentences of speakers from the
same village may result in conflicting evidence from the extracted
features. However, the dialect features should be the same for all
speakers.

In addition, the samples are normalized to account for
