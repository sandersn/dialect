...
have identical feature sets, unlike Ger\v{s}i\'c's $d$, for example,
...
\end{array} \right.}
  ...
  in this chapter \cite{lin91}. The difference is that WIV substracts
  ...
  facilitate the presentation, but typically it's used like an ad-hoc
...
in chapter \ref{questions-chapter}, syntax distance must be approached
quite differently than phonological distance. Syntactic corpora can be
built quickly by automatically annotating raw text, so it is easier to
build a large syntactic corpus than a phonological one; phonological
annotation does not yet have a method for automatic
annotation. However, automatic annotators, while faster, cannot
compete with human annotators in quality of annotation.  For example,
the principal difference between present phonological and syntactic
corpora is that phonology data is word-aligned, while syntax data is
not sentence-aligned. This difference leads syntactic approaches
naturally to statistical measures over large amounts of data rather
than more sensitive measures that operate on small corpora.
...
difference between the two age groups. The features that contributed
most to the difference were those in the older age group that are
unexpected in English. For example, some important features for the
older age group involved determiners, which English has but Finnish
does not. The features showed underuse of determiners, as well as
overuse, probably due to hypercorrection. Interestingly, some of these
features occur in the younger age group, but not as often.  Nerbonne
\& Wiersma analyzed this pattern as interference from Finnish; the younger
age group learned English more completely with less interference
from Finnish.
...
The input is 30 interview sites (described in
section \ref{syntactically-annotated-corpus}). The sentences in each
site have their features extracted (the features are described in
section \ref{syntactic-features}). Optionally, only 1000 sentences are sampled
with replacement, but the site sizes, unlike the British interviews
...
variation in site size and sentence length (described in
...
site differences for ranking: the top-ranked feature set will be
the one that produces the highest number of significant distances
between sites. So, if a feature set finds significant differences
between 90 out of 100sites, its weight will be 0.90 and will be double
the weight for a feature set that only finds significant differences
between 45/100 sites.
...
site is plotted as a point on that dimension. The result is equation
\ref{cosmeasure} or the non-vectorized version in
\ref{cosmeasureiterative}.

\begin{equation}
  \frac{a\cdot b} {||a_i||||b_i||}
  \label{cosmeasure}
\end{equation}
...
inter-site distances to be compared more easily. The dendrogram that
binary hierarchical clustering produces allows easy visual comparison
of the most similar sites. An example is given in figure
...
function---for example, the average of distances between sites in
...
traditional dialectology measures are do not provide numeric distances
...
of the two sites, whereas the second removes it from the distance
between two sites. Both normalizations are similar in being of
