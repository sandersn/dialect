...
The Berkeley parser, as described in section \label{parsers}, can
...
% TODO: An example would be nice, but I can't get that until I
...
compared. He does this in place of the However, Wiersma also runs a third
...
This permutation removes the effect of frequency from the difference
of the two regions, much like the second normalisation removes
frequency from the distance of the two regions. As such, it is of
questionable value for our noisier data, but is still interesting for
analysis; if the data are clean {\it enough}, then it could give
better results than the non-frequency-normalised differences.

The summing proceeds as before.

Example goes here, it shouldn't be too hard, just base it off the
other one, with made up corpus sizes added.
