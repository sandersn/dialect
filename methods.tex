...
centrality to the sentence. % or something, this is still kind of % wrong
...
algorithm repeatedly merges speakers, it is
not always clear what the best clusters are. For example,
...
North/South clusters for most features. These clusters had less
...
determine what is a cluster and what is not. The clustering algorithm
itself is nothing but binary merges---there may even be only one
cluster if all speakers are evenly distributed around a single center.
...
To find the two closest groups, Ward's method is used
\cite{ward63}. At each merge step, this method evaluates every
possible binary merge. Each merge is given a score that
minimises the square of the distances between regions in the new
group. The best merge at that point is added, its child groups are
removed, and the process repeats until a singly rooted tree is
created. For $n$ regions, this takes $n-1$ iterations, because at each
step, two groups are merged.

\begin{figure}
  \[ \sum_{i=1}^n d_i^2 - \frac{1}{n}\bigg(\sum_{i=1}^n d_i\bigg)^2 \]
  \label{wards-equation}
  \caption{Ward's Objective Function}
\end{figure}

Ward's method is more complicated than some other common clustering
methods, but it usually finds small, round clusters, making it worth
the extra computer time necessary.  In contrast,
single-link distance, for example, compares only the two closest
elements of the two members of a possible merge. This is faster, but
is susceptible to creating thin, oval groups---even though the bulk of
a group may be distant, a single outlier usually leads to a bad
grouping, which recursively leads to further outliers.
...
between inter-connected sites \cite{mantel67}. Mantel's test much like
...
Multi-dimensional scaling (MDS) is an alternate approach to collapse the
high-dimensional distances to something easily interpretable. Instead
of creating a tree, multi-dimensional scaling scales the
high-dimensional data directly down to 3 dimensions, which can
then be represented using (R,G,B) colour triples.

It does this using Kruskal's method (CITE Kruskal). Kruskal's method
is \ldots uh \ldots described by L04 I guess.

\subsubsection{PCA}

I think L04 does it for me. Probably. (The L04 tutorial also mentions
...
\subsubsection{Grandparent PSGs}

\subsubsection{Dependency Arc Labels}
...
\subsubsection{KL divergence/dissimilarity}

\subsubsection{JS divergence/dissimilarity}
